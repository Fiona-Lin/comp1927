...
int mO3(int a[], int left, int right) ;
int randm(int a[], int left, int right);
...
int mO3(int a[], int left, int right) {
   swap(&a[right - 1], &a[(left + right) / 2]);
   if (a[left] > a[right - 1]) swap(&a[left], &a[right - 1]);
   if (a[right - 1] > a[right]) swap(&a[right - 1], &a[right]);
   if (a[left] > a[right - 1]) swap(&a[left], &a[right - 1]);
   return partition(a, left, right - 1);
}

int randm(int a[], int left, int right){
   int r = rand() % (right - left + 1);
   swap(&a[r + left ], &a[right]);
